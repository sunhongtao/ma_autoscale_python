...
mesos_urls = os.environ.get("MESOS_URLS", "10.254.9.55:5050;10.254.9.56:5050;10.254.9.57:5050")
...
def getinfo(db, marathon_name, app_id, scale_quotas):
    now_info = data_driver_v2.data_driver(db, marathon_name, app_id, scale_quotas)
    print ("DEBUG====get now info:",now_info)
    now_quota_info = {}
    try:
        if now_info['status'] == 0:
            now_instances = now_info['app_task_num']
            for quota in scale_quotas:
                now_quota_info[quota] = now_info[quota]
        else:
            now_quota_info = {}
            now_instances = 0
...
def scale(marathon, appid, rule):
    ...
    if 'scale_up' in rule:  # 扩容条件
		...
		t_tmp=t_tmp+1
                ...
                now_quota_info_temp, task_temp = getinfo("prometheus", marathon_ip, appid, scale_quotas)
		# print ("DEBUG====getinfo:",now_quota_info_temp, task_temp)
		if t_tmp > 3:
		    return
        ...
        if scale_up == rule['scale_up']['continue_period'] and int(
            ...
            cur.close()
            conn.close()
            print("+++++++++++++++++++这是{}第{}次超过指标=进入弹性扩容函数+++++++++++++++".format(appid,rule['scale_up']['continue_period']))
        ...
        if scale_down == rule['scale_down']['continue_period'] and int(
            ...
            print("-----------------------这是{}第{}次低于指标=进入弹性缩容函数--------------".format(appid,rule['scale_down']['continue_period']))
...
def test():
    ...
    while True:
        ...
        for rule in scale_rule:
    	    key = rule['marathon_name'] + rule['app_id'] + rule['scale_type']
	    if rule['scale_type'] == 'up':
		# print ("thread_times.has_key(key)",thread_times.has_key(key))
	        if thread_times.has_key(key):
		    print ("thread_times.has_key[key]",thread_times[key])
                    #marathon_name = rule['marathon_name']
                    #app_id = rule['app_id']
                    # print("DEBUG===========test:=====rule:", rule)
	    	    if math.ceil(time.time() - thread_times[key]) > rule['scale_up']['continue_period'] * rule['scale_up']['collect_period']:
                        scale_thread = threading.Thread(target=scale, args=(rule['marathon_name'], rule['app_id'], rule))
                        scale_threads.append(scale_thread)
	    	        thread_times[key] = time.time()
	        else:
	    	    scale_thread = threading.Thread(target=scale, args=(rule['marathon_name'], rule['app_id'], rule))
	    	    scale_threads.append(scale_thread)
	    	    thread_times[key] = time.time()
	    elif rule['scale_type'] == 'down':
		# print ("thread_times.has_key(key)",thread_times.has_key(key))
		if thread_times.has_key(key):
		    print ("thread_times.has_key[key]",thread_times[key])
	    	    if math.ceil(time.time() - thread_times[rule['marathon_name'] + rule['app_id'] + rule['scale_type']] > rule['scale_down']['continue_period'] * rule['scale_down']['collect_period']):
                        scale_thread = threading.Thread(target=scale, args=(rule['marathon_name'], rule['app_id'], rule))
                        scale_threads.append(scale_thread)
	    	        thread_times[key] = time.time()
	        else:
